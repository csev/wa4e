...
/**
 * Installation sanity checks for the WA4E home page only.
 */

function wa4e_sanity_die($title, $body_html) {
    error_log('WA4E sanity: '.$title);
...
if ( ! is_dir($tsugi_dir) ) {
    wa4e_sanity_die(
        ...
        '<p>This copy of Web Applications for Everybody expects a <code>tsugi</code> folder next to
        <code>index.php</code>, but that folder was not found.</p>
        <p>From your <code>wa4e</code> directory, check out Tsugi:</p>
        <pre>cd wa4e
git clone https://github.com/tsugiproject/tsugi.git tsugi</pre>
        ...
        <p>More detail: <a href="https://github.com/csev/wa4e/blob/master/README.md"
        target="_blank" rel="noopener noreferrer">WA4E README</a></p>'
    );
}
...
if ( count($missing) > 0 ) {
    wa4e_sanity_die(
        ...
        <p>From your <code>wa4e</code> directory, refresh the Tsugi checkout:</p>
        <pre>cd wa4e/tsugi
git pull
...
cp config-dist.php config.php</pre>
        <p>Edit <code>tsugi/config.php</code> for this site (see Embedded Tsugi settings in the README).</p>
